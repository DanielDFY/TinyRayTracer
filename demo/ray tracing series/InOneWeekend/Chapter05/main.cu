...
__global__ void render(
	...
	pixelBuffer[idx] = rayColor(r, hittableWorldObjList);
}

__global__ void createWorld(Hittable** hittableList, Hittable** hittableWorldObjList) {
	if (threadIdx.x == 0 && blockIdx.x == 0) {
		hittableList[0] = new Sphere(Vec3(0.0f, 0.0f, 1.0f), 0.5f);
		hittableList[1] = new Sphere(Vec3(0.0f, -100.5f, 1.0f), 100.0f);
		*hittableWorldObjList = new HittableList(hittableList, 2);
	}
}

__global__ void freeWorld(Hittable** hittableList, Hittable** hittableWorldObjList) {
...
int main() {
	...
	createWorld<<<1, 1>>>(hittableListPtr.get(), hittableWorldObjListPtr.get());
	...
	render<<<blockDim, threadDim>>>(pixelBufferPtr.get(), imageWidth, imageHeight, lowerLeftCorner, horizontal, vertical, origin, hittableWorldObjListPtr.get());
	checkCudaErrors(cudaGetLastError());
	checkCudaErrors(cudaDeviceSynchronize());
	...
	freeWorld<<<1, 1>>>(hittableListPtr.get(), hittableWorldObjListPtr.get());
	checkCudaErrors(cudaGetLastError());
	checkCudaErrors(cudaDeviceSynchronize());
